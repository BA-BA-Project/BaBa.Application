...
class MyPageGroupAdapter(
    ...
    class ViewHolder(
        ...
        fun bind(group: Group) {
            this.group = group
            adapter = GroupMemberAdapter { member -> showMemberInfo(group, member) }

            binding.groupName = group.groupName
            binding.rvGroupMembers.adapter = adapter
            if (group.family) {
                binding.description = binding.root.context.getString(R.string.family_group_description)
                ownerFamily = group.groupName
                adapter.submitList(babies + group.members.map { it.toMemberUiModel() })
            } else {
                binding.description = String.format(binding.root.context.getString(R.string.my_group_description), ownerFamily, group.groupName)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        return ViewHolder(view,showMemberInfo, editGroup, goToAddMemberPage, ownerFamily, babies)
    }
